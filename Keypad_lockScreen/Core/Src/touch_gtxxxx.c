...
/*
		...
		x2  = point_data[12] | (point_data[13] << 8);	//x coordinates
        y2  = point_data[14] | (point_data[15] << 8);	//y coordinates
        w2  = point_data[16] | (point_data[17] << 8);	//size
 	 	CTP_INFO("Touch down ! X2:%d, Y2:%d, W2:%d",  x2, y2, w2);
